...
class ChessEnvV2(gym.Env):
    ...
    def step(self, action):
        ...
        if self.current_player == WHITE:
            if not self.piece_is_on_board(self.board, -KING_ID):
                self.done = True
                reward += WIN_REWARD 
            elif not self.piece_is_on_board(self.board, KING_ID):
                self.done = True
                reward += LOSS_REWARD
        else:
            if not self.piece_is_on_board(self.board, KING_ID):
    ...
    def player_move(self, action):
        ...
        opponent = self.get_other_player(self.current_player)
        if self.is_king_in_check(opponent):
            if opponent == WHITE:
                self.white_king_is_checked = True
            else:
                self.black_king_is_checked = True
        else:
            if opponent == WHITE:
                self.white_king_is_checked = False
            else:
        ...
        if self.is_pawn_promotion(move):
            print(move)
            (x0, y0), (x1, y1) = move
            piece_id = self.board[x0][y0]
            promoted_piece = self.choose_promotion_piece(piece_id)
            new_state['board'][x1][y1] = promoted_piece
    ...
    def is_pawn_promotion(self, move):
        if isinstance(move, tuple) and len(move) == 2:
            (x0, y0), (x1, y1) = move
            piece_id = self.board[x0][y0]
            if abs(piece_id) == PAWN_ID:
                if (piece_id > 0 and x1 == 0) or (piece_id < 0 and x1 == 7):
                    return True
        return False
    
    def choose_promotion_piece(self, piece_id):
    ...
    def is_king_in_check(self, player):
        ...
        for x in range(8):
            for y in range(8):
                piece_id = self.board[x][y]
                if (player == WHITE and piece_id == KING_ID) or (player == BLACK and piece_id == -KING_ID):
                    king_position = (x, y)
                    break
            if king_position:
                break
        ...
        opponent_moves = self.get_possible_moves(player=opponent, attack=True)
